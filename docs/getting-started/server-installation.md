...
- **AUTO_FEED_ON** : if the values is set `true`, it will switch on **[Auto-feed](./tips.md/#auto-feed)** function that will archive experiments that have not been submitted through NOMAD. It will only work if IconNMR generates dataset name in a generic format XXX-XXX-XXX-username.
